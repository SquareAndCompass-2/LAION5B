...
def create_spark_session():
    ...
    spark = (
        ...
        .config("spark.driver.host", "172.31.12.184")
        .config("spark.driver.bindAddress", "172.31.12.184")
        .config("spark.executor.memory", "16G") # make sure to increase this if you're using more cores per executor
        .config("spark.executor.memoryOverhead", "8G")
        .config("spark.task.maxFailures", "100")
        .master("spark://172.31.12.184:7077") # this should point to your master node, if using the tunnelling version, keep this to localhost
        .appName("spark-stats")
        .getOrCreate()
    )
    return spark
